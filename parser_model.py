...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # x是embed_size维的n_features个词嵌入的串联
        # h=xW+b1
        self.embed_to_hidden_weight = nn.Parameter(torch.Tensor(self.n_features*self.embed_size,self.hidden_size).zero_())
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        self.embed_to_hidden_bias = nn.Parameter(torch.Tensor(self.hidden_size).zero_())
        nn.init.uniform_(self.embed_to_hidden_bias)

        self.dropout = nn.Dropout(p=dropout_prob)

        # l=hU+b2
        self.hidden_to_logits_weight = nn.Parameter(torch.Tensor(self.hidden_size,self.n_classes).zero_())
        nn.init.xavier_uniform_(self.hidden_to_logits_weight)
        self.hidden_to_logits_bias = nn.Parameter(torch.Tensor(self.n_classes).zero_())
    ...
    def embedding_lookup(self, w):
        ...
        x = self.embeddings.index_select(0,w.view(-1)) # view(-1)表示把w变成一维向量 此时选取w中所有的indices对应的词嵌入
        x = x.view(w.shape[0],-1) # 串联，batch_size表示串联的个数
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        h = F.relu(torch.matmul(x,self.embed_to_hidden_weight) + self.embed_to_hidden_bias) # h=ReLu(xw_1+b_1)
        self.dropout(h) # dropout层
        logits = torch.matmul(h,self.hidden_to_logits_weight) + self.hidden_to_logits_bias # l=hw_2+b_2
        ### END YOUR CODE
        return logits
