...
def minibatch_parse(sentences, model, batch_size):
    ...
    partial_parses = [PartialParse(sentence) for sentence in sentences] # 每个元素是一个等待处理的对象
    unfinished_parses  = partial_parses[:] # 浅拷贝
    while len(unfinished_parses) > 0:
        minibatch = unfinished_parses[0:batch_size]
        while len(minibatch) > 0:
            transitions = model.predict(minibatch)  # 每个句子走一步形成的操作集合
            for i in range(len(transitions) - 1, -1, -1):  # 有删除操作需要倒序遍历
                minibatch[i].parse_step(transitions[i])
                if len(minibatch[i].buffer) == 0 and len(minibatch[i].stack) == 1:
                    del minibatch[i]
        unfinished_parses = unfinished_parses[batch_size:] # 去除已经处理完的batch
    # 把每个句子形成的依赖整合到一个集合中
